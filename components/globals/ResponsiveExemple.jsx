...
export function ResponsiveExemple({ children = {} }) {
  ...
  const responsiveFontSizeByWidth = (width) => {
    const widthUnit = (width / 100);

    switch (true) {
      case (Number(width) >= 860):
        return (widthUnit * 1.7);
      case (Number(width) <= 860):
      default:
        return (widthUnit * 2.5);
    }
  }
  ...
  return (
    <SafeAreaView style={{
      ...
      <View style={{
        ...
        <Text style={{
          fontSize: responsiveFontSizeByWidth(width)
